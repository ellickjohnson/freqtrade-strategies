...
class BreakoutMomentum(IStrategy):
    ...
    volume_multiplier = DecimalParameter(
        1.4, 2.5, default=1.8, space="buy", optimize=True
    )
    ...
    candle_body_pct = DecimalParameter(
        0.5, 0.8, default=0.6, space="buy", optimize=True
    )
    trailing_start = DecimalParameter(
        0.005, 0.015, default=0.008, space="sell", optimize=True
    )
    ...
    minimal_roi = {"0": 0.04, "20": 0.02, "60": 0.01, "120": 0.005}
    max_open_trades = 3
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["highest_high"] = (
            dataframe["high"].rolling(window=self.BREAKOUT_PERIOD).max()
        )
        ...
        dataframe["rsi_valid"] = (dataframe["rsi"] > self.RSI_MIN) & (
        ...
        dataframe["bullish_candle"] = (dataframe["close"] > dataframe["open"]) & (
            dataframe["body_pct"] > self.CANDLE_BODY
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        dataframe.loc[
            (dataframe["rsi"] > 75) | (dataframe["close"] < dataframe["ema_200"]),
            "exit_long",
        ] = 1
        return dataframe
