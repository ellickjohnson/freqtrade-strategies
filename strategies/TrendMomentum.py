...
class TrendMomentum(IStrategy):
    ...
    volume_multiplier = DecimalParameter(1.2, 2.0, default=1.4, space='buy', optimize=True)
    ema_period = CategoricalParameter([20, 50], default=50, space='buy', optimize=True)
    candle_body_pct = DecimalParameter(0.4, 0.7, default=0.5, space='buy', optimize=True)
    max_duration_hours = DecimalParameter(1.5, 4.0, default=3.0, space='sell', optimize=True)
    profit_target = DecimalParameter(0.02, 0.08, default=0.04, space='sell', optimize=True)
    trailing_start = DecimalParameter(0.005, 0.02, default=0.01, space='sell', optimize=True)
    ...
    def protections(self):
        return [
            {"method": "CooldownLookback", "stop_duration_candles": 5},
        ]
    ...
    def do_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for pair, tf in inf_pairs:
            if pair == metadata['pair'] and tf == '1h':
                ...
                dataframe = merge_informative_pair(dataframe, informative, self.timeframe, tf, 
                                                   ffill=True, drop_nan=True)
        return dataframe
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        ema_200_1h_col = 'ema_200_1h_1h' if 'ema_200_1h_1h' in dataframe.columns else 'ema_200_1h'
        if ema_200_1h_col in dataframe.columns:
            dataframe['above_ema200_1h'] = dataframe['close'] > dataframe[ema_200_1h_col]
        else:
        ...
        dataframe['macd_increasing'] = dataframe['macdhist'] > dataframe['macdhist'].shift(1)
        dataframe['macd_bullish'] = dataframe['macd_positive'] & dataframe['macd_increasing']
        ...
        dataframe['momentum_alignment'] = (
            dataframe['rsi_not_oversold'] & 
            dataframe['macd_bullish']
        )
        dataframe['price_position'] = dataframe['above_ema50'] | dataframe['ema_cross_up']
        
        return dataframe

    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Entry conditions - trend following with momentum confirmation."""
        conditions = [
            # 1. Trend: Price above 1h EMA200
            dataframe['above_ema200_1h'],
            # 2. Price position: Above EMA50 OR EMA crossover bullish
            dataframe['price_position'],
            # 3. ADX > 20 (trend strength)
            dataframe['strong_trend'],
            # 4. RSI > 45 (not oversold)
            dataframe['rsi_not_oversold'],
            # 5. MACD positive and increasing
            dataframe['macd_bullish'],
            # 6. Volume spike
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Exit conditions - momentum exhaustion."""
        conditions = [
            # RSI overbought
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
                            ...
                            **kwargs) -> bool:
        """Final confirmation - reject weak entries."""
        dataframe, _ = self.dp.get_analyzed_dataframe(pair, self.timeframe)
        last_candle = dataframe.iloc[-1]
        
        # Reject if volume below average
        if float(last_candle['volume']) < float(last_candle['volume_mean']):
            return False
        
        # Reject if ADX too low
        if float(last_candle['adx']) < 15:
            return False
        
        # Reject if RSI is oversold (reversal trap)
        if float(last_candle['rsi']) < 35:
            return False
